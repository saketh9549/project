...
export default function VideosCatalog({
  ...
  // Selected state and menu dropdown helpers
  ...
  const renderVideoItem = (v, indent = false) => {
    ...
    const handleClick = () => {
      if (isProcessing) return; // Disallow selecting a video that is still processing
      onSelectVideo(v);
    };
          ...
          <div className="flex items-center gap-1.5 shrink-0 relative">
            ...
            {activeMenuVideoId === v.id && (
              <>
                ...
                />
                <div className="absolute right-0 top-full mt-1.5 w-48 bg-gray-950/95 border border-white/10 rounded-xl shadow-2xl z-50 p-1.5 backdrop-blur-md">
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
          ...
          {showNewFolderForm && (
            <form onSubmit={handleCreateFolder} className="flex gap-2 items-center animate-fade-in bg-white/5 p-2 rounded-xl border border-white/5">
              <input
                ...
                className="flex-grow bg-gray-900/60 border border-white/10 rounded-lg px-2.5 py-1 text-xs text-white focus:outline-none focus:border-indigo-500 transition-all placeholder-gray-700"
              ...
              <button
                ...
                className="bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs px-2.5 py-1 rounded-lg transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="text-gray-400 hover:text-white text-xs px-1 py-1 cursor-pointer"
              >
                Cancel
              </button>
            </form>
      ...
      <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-2 min-h-0">
        {!videos.some(v => v.upload_status === 'indexed') && playlists.length === 0 ? (
                  ...
                  {isExpanded && (
                    <div className="flex flex-col gap-1.5 mt-1 pl-2 border-l border-white/5 ml-3.5 pb-1">
                      {folderVideos.length === 0 ? (
                        ...
                        folderVideos.map((v) => renderVideoItem(v, false))
                      )}
                    </div>
                  )}
                </div>
